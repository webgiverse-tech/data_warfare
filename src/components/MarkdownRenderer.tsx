...
// Define a type for the code component props to include 'inline'
interface CodeProps extends React.HTMLAttributes<HTMLElement> {
  node?: any; // Added node property
  inline?: boolean;
  className?: string;
    ...
    <Card className="bg-dw-background-glass border border-dw-accent-primary/30 shadow-lg shadow-dw-accent-primary/10 p-8 rounded-lg relative overflow-hidden">
      ...
      <div className="relative z-10">
        <ReactMarkdown
          ...
          components={{
            h1: ({ node, ...props }) => <h1 className="text-4xl font-heading text-dw-accent-primary mb-6 mt-10 border-b border-dw-accent-primary/30 pb-2" {...props} />,
            ...
            code: ({ node, inline, className, children, ...props }: CodeProps) => ( // Use CodeProps
              <code className={`font-mono text-sm ${inline ? 'bg-dw-background-glass px-1 py-0.5 rounded' : 'block bg-dw-background-deep border border-dw-accent-secondary/30 p-4 rounded-md my-4 overflow-x-auto whitespace-pre-wrap'} ${className || ''}`} {...props}>
                {children}
              </code>
            ),
